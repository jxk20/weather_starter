...
function TileShell({ icon, title, className = '', children }: TileShellProps) {
  return (
    <section
      className={`flex flex-col gap-3 rounded-[var(--radius-card)] border border-[color:var(--border)] bg-[color:var(--surface)] p-[var(--space-card)] backdrop-blur-[var(--blur-card)] ${className}`}
    >
      <header className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[color:var(--text-4)]">
        {icon}
        <span>{title}</span>
      </header>
...
export function AirQualityTile({ weather }: WeatherProps) {
  ...
  return (
    ...
    >
      <div className="text-4xl font-light leading-none text-[color:var(--text-2)]">{psi}</div>
      <div className="mt-1 text-base text-[color:var(--text-2)]">
    ...
    <TileShell icon={<WindIcon />} title="Wind" className="col-span-2">
      <div className="grid grid-cols-[1fr_auto] items-center gap-4">
        <ul className="space-y-2 text-sm">
          <li
            className={`flex justify-between ${hasDirection ? 'border-b border-[color:var(--border-soft)] pb-2' : ''}`}
          >
            <span className="text-[color:var(--text-3)]">Wind</span>
            <span className="tabular-nums text-[color:var(--text-2)]">{speed} km/h</span>
          </li>
          {hasDirection && (
            <li className="flex justify-between">
              <span className="text-[color:var(--text-3)]">Direction</span>
              <span className="tabular-nums text-[color:var(--text-2)]">{direction}&deg;</span>
            </li>
          )}
        </ul>
...
function Compass({ speed, direction }: CompassProps) {
  ...
  return (
    <div className="relative h-20 w-20 rounded-full border border-[color:var(--border)] bg-[color:var(--surface-soft)]">
      <span className="absolute left-1/2 top-1 -translate-x-1/2 text-[10px] text-[color:var(--text-4)]">
        N
      </span>
      <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[10px] text-[color:var(--text-4)]">
        E
      </span>
      <span className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[10px] text-[color:var(--text-4)]">
        S
      </span>
      <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[10px] text-[color:var(--text-4)]">
        W
      </span>
      {hasDirection && (
        <div
          className="absolute left-1/2 top-2 h-7 w-px origin-[50%_30px] -translate-x-1/2 rounded-full bg-[color:var(--text-3)]"
          style={{ transform: `translateX(-50%) rotate(${direction}deg)` }}
        />
      )}
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-base font-light leading-none text-[color:var(--text-2)]">
          {speed}
        </span>
        <span className="text-[10px] text-[color:var(--text-4)]">km/h</span>
      </div>
    </div>
  );
}
...
export function UVTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<SunIcon className="h-3.5 w-3.5" />} title="UV Index">
      <div className="text-4xl font-light leading-none text-[color:var(--text-2)]">
        {formatNumber(weather?.uv_index)}
      </div>
      <div className="mt-1 text-base text-[color:var(--text-2)]">{uvLabel(weather?.uv_index)}</div>
      <ScaleBar
...
export function AveragesTile({ weather }: WeatherProps) {
  return (
    <TileShell icon={<TrendIcon />} title="Forecast High">
      ...
      <div className="mt-1 text-xs leading-snug text-[color:var(--text-3)]">
        Today&apos;s high from the 24-hour forecast.
      </div>
      <ul className="mt-3 space-y-1 text-xs text-[color:var(--text-3)]">
        <li className="flex justify-between border-t border-[color:var(--border-soft)] pt-1.5">
          <span>Today</span>
