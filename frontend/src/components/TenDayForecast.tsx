...
export function TenDayForecast({ weather }: TenDayForecastProps) {
  ...
  return (
    <section className="rounded-[var(--radius-card)] border border-[color:var(--border)] bg-[color:var(--surface)] backdrop-blur-[var(--blur-card)]">
      <header className="flex items-center gap-2 border-b border-[color:var(--border-soft)] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-[color:var(--text-4)]">
        <CalendarIcon />
      ...
      <ul className="divide-y divide-[color:var(--divider)]">
        {days.length > 0 ? (
          days.map((day, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="tabular-nums text-[color:var(--text-4)]">
                  {formatTemperature(day.temperature_low_c)}
                </span>
                <div className="relative h-1.5 rounded-full bg-[color:var(--surface-input)]">
                  {offsets && (
                    <div
                ...
                {day.forecast && (
                  <span className="col-span-5 -mt-1 truncate text-xs text-[color:var(--text-4)]">
                    {day.forecast}
                  </span>
                )}
              </li>
          ...
          <li className="px-4 py-4 text-sm text-[color:var(--text-4)]">Forecast unavailable.</li>
        )}
      </ul>
    </section>
  );
